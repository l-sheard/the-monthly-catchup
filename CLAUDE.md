...
**Newsletter compile + send** (`apps/api/src/lib/newsletter.ts`): `sendNewsletterForCycle` gathers every member's answers + attached media + meetup suggestions for a cycle, renders one HTML email (styled via `apps/api/src/lib/email-theme.ts` — the same paper/coral/mono palette as the mobile app, kept in sync by hand since there's no shared token file), and sends a copy to each member independently via Resend — one member's send failing (e.g. Resend's dev-sandbox restriction to the account owner's own address) doesn't block the others. Also available on demand via `POST /cycles/:id/send-newsletter`, which is what the auto-send job below effectively does at the deadline.

**Scheduled jobs** (`apps/api/src/index.ts`'s `scheduled` export, driven by a Cloudflare Cron Trigger — `0 6 * * *`, see `wrangler.jsonc`). No third-party job platform — two straightforward scheduled jobs didn't justify one:
- `openCyclesForToday` (`apps/api/src/jobs/open-cycles.ts`) opens a cycle once the current month's final week starts (a fixed number of days before the month's last day — see `OPEN_DAYS_BEFORE_MONTH_END`), so members are reflecting on a month that's essentially over rather than guessing at days that haven't happened yet. The deadline is always that month's actual last day, computed per-month (leap Februaries, 30- vs 31-day months included) rather than a fixed day-of-month. Self-healing by construction: it no-ops before the window, then for every day inside it tries to open a cycle for every group that doesn't have one yet for this month/year — a missed cron invocation on the exact open day just gets picked up the next day, and the per-group "does a cycle already exist" check (backstopped by the DB's `unique(groupId, month, year)`) makes re-running safe. Also sends the "this month is open" reminder email (`apps/api/src/lib/reminder-email.ts`) to every member the moment a cycle opens.
- `sendNewslettersPastDeadline` (`apps/api/src/jobs/send-newsletters.ts`) sends any cycle that's still `status: 'open'` whose `deadlineAt` has passed, rather than checking "is today the deadline day" — the deadline column already encodes that. Same self-healing shape as the job above: `sendNewsletterForCycle` flips status to `'sent'` as its last step, so a delayed cron run just catches up, and nothing gets double-sent.
