...
// Nested under cycles/[id]/ rather than a query param or modal — it's a
// real destination (deep-linkable, has its own back button) reached from
// the cycle overview's "Fill out this month" card, not a variant of that
// screen. Its own inline auth guard, same reasoning as the overview.
export default function CycleAnswerScreen() {
  ...
  // Same pattern as the overview screen: apiFetch's identity isn't stable
        ...
        {data && (
          <View className="w-full max-w-xl gap-4">
            <View className={`gap-5 p-5 ${CARD}`}>
              ...
              {savedAt && <Text className="text-center font-mono text-sm text-emerald-600">Saved!</Text>}
            </View>
